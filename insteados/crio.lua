...
main = R {
	...
	до своего отсека и уйти в хибернацию...^^
	{xwalk(r7)|Дальше}]];
}
...
port = O {
	...
	act = function(s)
		...
		p [[Тут нужен разьем, которого у меня нет!!!]]
		_port = true;
	end;
	obj = { };
}:disable()
...
flame = O {
	...
	use = function(s, w)
		...
		if w == rat then
			...
			fl._pena = fl._pena + 5
			p [[Мои манипуляторы не привыкли управлятся огнетушителем,
...
p2 = room {
  ...
  pxa = {
	...
	dscs = { 
	    ...
	    "Внимание! Послышался женский голос. Целостность программы нарушена!!! Аварийный выход из хибернации через 60 секунд!!!",
	    "Что я наделал?",
	};
	dsc = function(s)
		p (s.dscs[s._s])
