...
liftDoor = obj {
    nam = "двери лифта",
    act = [[Конструкция таких дверей не сильно изменилась за прошедшие века. При открывании дверь уходит в стену. Между стеной и дверью есть зазор.]],
};
...
^
Раздумывая о происшедшем, я не сразу заметил, что сзади подкрадывается огромная {rat|крыса} размером с дога,
...
v_7 = room {
    ...
    dsc = function(s)
p [[
Я с силой вталкиваю швабру в узкий зазор между дверью лифта и стеной.
На какое-то время {zombie2|сумасшедший} заблокирован в кабине лифта, но по грохоту, раздающемуся изнутри, я понимаю,
что надолго это его не задержит.
^^
]];
...
p [[
Мой взгляд скользит по стенам, по лужам крови на полу, останавливается на теле Александра. Верхняя часть его тела лежит в
коридоре, нижняя -- за дверью отсека K007. В груди Александра всё ещё торчит {shpritz|шприц} с остатками адреналина.
^
{zombie2|Безумец} справился с дверью лифта и приближается ко мне.
^^
]];
