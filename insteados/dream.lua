...
fence=obj{
   ...
   act=function()
	 ...
	 place(neighbor)
	 return 'За забором я увидел соседа.'
      else
...
neighor_dlg=dlg{
   ...
   enter='Как только я обратил на соседа внимание, как он повернулся ко мне и сказал:^^-- Привет. Видал что на площади делается?',
   phr={
...
hole=obj{
   ...
   act=function(s)
	 ...
	 if not path(vroom('В туннель','tunnel')) then
      ...
      return 'Абсолютно прямой туннель, в конце которого что-то ослепительно сияет.'
   end,
}
...
event=room{
   ...
   dsc=function(s)
	 ...
	 v=v..'^^Неторопливым движением она склонилась к молоденькой девушке, что живёт недалеко от меня.'
      ...
      if s._n>1 then
	 v=v..'^^Ветка всё ближе и ближе подбирается к девушке. Похоже, никто кроме меня не замечает этого.'
      end
      if s._n>2 then
