...
main2 = R {
  ...
  pxa = {
	...
	dsc = [[Наша цель близка. Планета, к которой мы мчались на околосветовой скорости всю эту бесконечную 
		...
		которые наши компьютеры дешифровали как ПРЕДУПРЕЖДЕНИЕ.^^{xwalk(station)|Дальше}]];
...
s3 = R {
  ...
  pxa = {
	...
	way = { vroom('Налево', s4x), vroom('Направо', s4), vroom('К шлюзу', s2) };
}
...
door = O {
	...
	act = function(s)
		if not s._open then
			p [[Я приблизился к двери и она с зловещим шипением отъехала в сторону.]];
			s._open = true
		else
...
hend = R {
  ...
  act= function(s)
	...
	dsc = [[
		...
		-- А что со станцией?^
		-- Не представляет опасности. В ее искуственном интеллекте произошел какой-то сбой.^
		-- А сигнал?^
