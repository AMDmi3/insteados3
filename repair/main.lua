...
redwire = obj {
  ...
  ,dsc = function(s)
     if s._set == "" and have(red) then
       return "Красный провод отсоединён.";
     elseif s._set == "" then
       return "{Красный провод} отсоединён.";
     end
...
endgame = room {
  ...
  ,dsc = "Ну, наконец-то! И что только творится в электрических мозгах у этих роботов?! Как можно было так подключить энергические контуры?^Довольный, я вернулся в свой крио-блок и столкнулся там с потрёпанным на вид роботом.^-- Были проблемы с освещением, -- сказал робот. -- Но я всё починил. Волноваться не о чем.^И быстро вышел в коридор -- странной прихрамывающей походкой.^^"..txtc("КОНЕЦ?")
}
