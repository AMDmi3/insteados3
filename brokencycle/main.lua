...
main=room{
   nam='Капсула',
   dsc='Сознание вернулось. А в месте с ним верулись мрак и холод.',
...
cell21=obj{
   ...
   act='Мне не открыть эту дверь руками.',
}
...
liftbuttons=obj{
   nam='Кнопки',
   dsc='На стене лифта находсятся {кнопки}.',
   act=function()
...
sparehand=obj{
   ...
   use=function(s,w)
	 ...
	 drop(s,cell20)
	 return 'Я положил запасню руку рядом с дверью отсека №20.'
      end
   end,
}
...
last=room{
   nam='Резервный криоблок',
   dsc='На последок, оглядев криоблок, я закрыл капсулу и погрузился в долгий сон, наполненный виртуальной жизнью...',
   act=function()
      walk(epilogue)
   end,
   obj={vobj('next','{Далее}')},
