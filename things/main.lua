...
global { broken_panel = false; was1 = false; was2 = false;}
main = room{
	nam = "Неизбежные вещи",
	...
	act = function(s)
		if s._lvl == 1 then
			...
			if s._lvl-3 < #s.desc then
				...
				p "- Эй, ты живой? - доносится до меня его голос. ^ Я закрываю глаза, в надежде обмануть. Но он тормошит меня и от боли веки сами распахиваются. Мучитель хмурится.";
			end
		end
	end,
	used = function(s, w)
...
space3 = obj{
	...
	act = function(s)
		if have "burn_up" and broken_panel then	walk "end_of_story";
			...
			p "Судя по открывшемуся виду, ракета летит куда-то прочь от ядра Стройки... Пространство тут не так загажено выхлопом, да и кораблей куда меньше, чем я привык...^ Эй, а это что?! Одно {anemy_ship|судно} словно зависло в пространстве. А это может означать только одно - оно следует той же траекторией, с той же скоростью...";
		end
	end,
};
jet = obj{
...
miracle = obj{
	nam = "Чудо",
	dsc = "Тело совершенно не ощущается. Голова слегка кружится. У меня дырка в груди. Легкие потихоньку заполняются жидкостью. Но я жив!^ Упираюсь взглядом в самую яркую звезду в поле зрения. Возношу безмолвную молитву мирозданию и партии за это маленькое, плановое {чудо}.^^",
	tak = function()
	...
	inv = "Выкарабкаюсь - лично пожму руку изобретателю изобретателю.";
	use = function(s, w) s:used(w) end;
	used = function(s, w)
		if w == corpse then
...
end_of_story = room{
	...
	dsc = function(s)
		local fin = {
			...
			""; "эээ"; "1"; "2";  "..."; "Сфероид! Это они!"; "Проходит мимо! Меня не заметили!";
			"..."; "1254; a"; "5488;  эпсилон"; "...";
